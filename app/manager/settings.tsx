...
export default function ManagerProfile() {
  ...
  const refresh = useCallback(async (isManual = false) => {
    ...
    try {
      const { data } = await fetchWithCache("/auth/me");
      setProfile(data);
  ...
  async function submitVerification() {
    ...
    setVerifying(true);
    try {
      const { data } = await api.patch("/auth/me/verify", {
      ...
      Alert.alert(
        "✅ Verified",
        "Your identity has been verified. Farmers can now see your verified status.",
      );
    } catch (err: any) {
      Alert.alert(
        "Verification failed",
    ...
    <SafeAreaView className="flex-1 bg-[#FCF9F8]">
      ...
      >
        ...
        <Text className="text-gray-500 mt-1 mb-5">
          Manage your manager account and verification.
        </Text>
            ...
            <View className="flex-row items-start justify-between">
              ...
              {profile.verified ? (
                <View className="flex-row items-center gap-1 bg-[#A3E635]/20 rounded-full px-3 py-1">
                  ...
                  <Text className="text-[#A3E635] text-[11px] font-black uppercase">
                    Verified
                  </Text>
                </View>
              ) : (
                <View className="bg-amber-400/20 rounded-full px-3 py-1">
                  <Text className="text-amber-300 text-[11px] font-black uppercase">
                    Unverified
                  </Text>
                </View>
              )}
        ...
        {profile ? (
          <View className="bg-white rounded-2xl p-4 border border-gray-200 mb-4">
            <View className="flex-row items-center gap-2 mb-3">
              ...
              <Text className="text-[11px] text-gray-500 uppercase font-black flex-1">
                Identity Verification
              </Text>
              {profile.verified ? (
            ...
            {profile.verified ? (
              <>
                <View className="flex-row items-center gap-2 bg-[#E7F5EE] rounded-xl px-4 py-3 mb-2">
                  <MaterialIcons
                    ...
                    color="#2A5C43"
                  />
                  <Text className="text-[#2A5C43] font-black flex-1">
                    Identity Verified
                  </Text>
                </View>
              ...
              <>
                <Text className="text-gray-600 text-sm mb-3">
                  Enter your Kenyan National ID number to verify your manager
                  identity. This will be visible to farmers.
                </Text>
                <EditField
              ...
              <>
                <Text className="text-gray-500 text-sm mb-3">
                  Verify your identity with your National ID so farmers can
                  trust your cooperative.
                </Text>
                <Pressable
                  onPress={() => {
                    setVerifyMode(true);
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <View className="py-2 border-b border-gray-100">
      <Text className="text-[11px] text-gray-500 uppercase font-bold">
        {label}
      </Text>
